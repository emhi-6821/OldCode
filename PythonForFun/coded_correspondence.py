...
#Message decoder function, takes offset and the message, returns the decoded message, offset is added to coded message to find decoded message
def decoder(message, offset):
    message_decoded = ""
    for i in message:
        if i in alph:
            indx = alph.find(i)
            message_decoded += alph[(indx+offset)%26]
        else:
            message_decoded += i
...
#Message coder function, takes uncoded message and offset to return coded message, subtract offset to code
def coder(message, offset):
    message_coded = ""
    for i in message:
        if i in alph:
            indx = alph.find(i)
            message_coded += alph[(indx-offset)%26]
        else:
            message_coded += i
    return message_coded

print(coder("Vishal, hi. Can you read this?", 10))
...
print(decoder(message_1, 10))
print(decoder(message_2, 14))
...
message_3 = "dfc aruw fsti gr vjtwhr wznj? vmph otis! cbx swv jipreneo uhllj kpi rahjib eg fjdkwkedhmp!"
keyword = "friends"

def v_decoder(message_coded, keyword):
    new_msg = ""
    ...
    for i in range(len(new_msg)):
        keyword_long += keyword[i%(len(keyword))]
    print(keyword_long)
    
    decoded_nospc = ""
    for i in range(len(new_msg)):
        decoded_nospc += alph[(alph.find(new_msg[i])-(alph.find(keyword_long[i])))% 26]
    print(decoded_nospc)
    
    decoded_message = ""
    count = 0
    for i in range(len(message_coded)):
        if message_coded[i] in alph:
            decoded_message += decoded_nospc[count]
            count += 1
        else:
            decoded_message += message_coded[i]
    return decoded_message


print(v_decoder(message_3, keyword))
...
def v_coder(message, keyword):
    ...
    coded_nospc = ""
    for i in range(len(new_msg)):
        coded_nospc += alph[(alph.find(new_msg[i])+(alph.find(keyword_long[i])))% 26]
    print(coded_nospc)
    ...
    for i in range(len(message)):
        if message[i] in alph:
            coded_message += coded_nospc[count]
            count += 1
        else:
            coded_message += message[i]
    return coded_message

print(v_coder("barry is the spy", "dog"))
