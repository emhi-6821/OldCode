...
def available_on_night(gamers_list, day):
    ...
    for gamer in gamers_list:
        if day in gamer['availability']:
            gamers_attending.append(gamer['name'])
...
form_emial = """Dear {name},

Please join us for a {game} night on {day_of_week}!
...
def send_email(gamers_who_can_attend, day, game):
    for gamer in gamers_who_can_attend: 
        print(form_emial.format(name = gamer, game = game, day_of_week = day ))
...
unable_to_attend = []
for i in gamers:
    if i.get('name') in attending_game_night:
        continue
    else:
...
calculate_availability(unable_to_attend, second_night_availability)   
